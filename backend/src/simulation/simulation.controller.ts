...
export class SimulationController {
  ...
  async simulateBuyerCreateListing(
    ...
    return this.prisma.buyListing.create({
      data: {
        ...
        neededBy: dto.neededBy ? new Date(dto.neededBy) : new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
        status: 'ACTIVE',
      },
    });
  }
  ...
  async simulateSellerAcceptOffer(
    ...
    const quantity = offerData?.quantity || negotiation.tradeSeller.requestedQuantity;
  ...
  async simulateSellerCounterOffer(
    @Param('userId') userId: string,
    @Body() dto: { negotiationId: string; counterPrice: number; counterQuantity?: number }
  ) {
    const negotiation = await this.prisma.offerNegotiation.findUnique({
      where: { id: dto.negotiationId },
      include: {
  ...
  async simulateTransporterBid(
    @Param('userId') userId: string,
    @Body() dto: { transportRequestId: string; bidAmount: number; estimatedDuration: number; vehicleType?: string; vehicleCapacity?: number }
  ) {
    const user = await this.prisma.user.findUnique({
      where: { id: userId },
    });
  ...
  async sendOffersToFarmers(
    ...
    return this.simulationService.sendOffersToFarmers(dto.tradeOperationId, dto.offers);
  }

  @Post('admin/accept-counter-offer')
  @ApiOperation({ summary: 'Admin accepts farmer counter-offer' })
  @HttpCode(HttpStatus.OK)
  async adminAcceptCounterOffer(
    @Body() dto: { negotiationId: string }
  ) {
  ...
  async assignInspector(
    ...
    return this.simulationService.assignInspector(dto.tradeOperationId, dto.inspectorId);
  }
  ...
  async createAndAcceptTransportBid(
    ...
    return this.simulationService.createAndAcceptTransportBid(dto.tradeOperationId, dto);
  }

  @Post('admin/complete-trade')
  @ApiOperation({ summary: 'Complete trade operation' })
  @HttpCode(HttpStatus.OK)
  async completeTradeOperation(
    @Body() dto: { tradeOperationId: string }
  ) {
    return this.simulationService.completeTradeOperation(dto.tradeOperationId);
  }

  @Post('admin/create-transport-request')
  @ApiOperation({ summary: 'Create transport request without bid (for bidding competition)' })
  @HttpCode(HttpStatus.CREATED)
  ...
  async selectTransportBid(
    ...
    return this.simulationService.adminSelectTransportBid(dto.transportRequestId, dto.bidId);
  }

  @Post('admin/update-pricing')
  @ApiOperation({ summary: 'Update negotiation pricing (quality dispute adjustments)' })
  @HttpCode(HttpStatus.OK)
  async updatePricing(
    @Body() dto: {
