...
export class SimulationService {
  ...
  async getFullTradeState(tradeOperationId: string) {
    ...
    const activeTransport =
      ...
      (operation.transportRequest.status === TransportRequestStatus.ASSIGNED ||
        operation.transportRequest.status ===
    ...
    return {
      ...
      state: {
        ...
        inspections: {
          ...
          pending: operation.inspections.filter((i) => i.status === "PENDING")
            .length,
  ...
  async createAndAcceptTransportBid(
    ...
    const totalWeight = tradeOp.sellers.reduce(
      (sum, s) => sum + Number(s.agreedQuantity || 0),
      0,
    );
  ...
  async transporterSubmitBid(
    ...
    return {
      transportBid,
      message: `Bid submitted: €${data.bidAmount} (${data.estimatedDuration}h)`,
    };
  }
  ...
  async adminSelectTransportBid(transportRequestId: string, bidId: string) {
    ...
    return {
      transportJob,
      winningBid,
      message: `Selected bid from transporter for €${winningBid.bidAmount}`,
    };
  }
   ...
   */
  async updateNegotiationPricing(
    ...
    if (role === UserRole.FARMER && data?.companyName) {
      await this.prisma.company.create({
        data: {
          userId: user.id,
          legalName:
            data.companyName || data.farmName || `Test Farm ${timestamp}`,
        },
      });
    }
