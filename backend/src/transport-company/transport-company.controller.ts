...
export class TransportCompanyController {
  constructor(private readonly transportCompanyService: TransportCompanyService) {}
  ...
  async getMyCompany(@Request() req: any) {
    const company = await this.transportCompanyService.getCompanyByAdminId(req.user.userId);
    if (!company) {
      return {
        success: false,
  ...
  async updateCompanyProfile(
    ...
    const updated = await this.transportCompanyService.updateCompanyProfile(id, updates);
    return {
      success: true,
      data: updated,
    };
