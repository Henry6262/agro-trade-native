...
export class TransportBiddingController {
  ...
  @Post("requests/auto")
  @ApiOperation({
    summary: "Automatically create a transport request for a trade operation",
  })
  @ApiResponse({
    status: HttpStatus.CREATED,
    description: "Transport request created successfully",
  ...
  async createTransportBid(
    ...
    const canBid = await this.permissionsService.canSubmitTransportBids(
      req.user.id,
    );
    if (!canBid) {
  ...
  async acceptTransportBid(@Param("id") id: string, @Request() req: any) {
    return await this.transportBiddingService.acceptTransportBid(
      id,
      req.user.id,
    );
  }
