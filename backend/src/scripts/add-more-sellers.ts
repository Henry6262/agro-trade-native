  ...
  for (const product of products) {
    ...
    for (let i = 0; i < Math.min(3, sellers.length); i++) {
      ...
      if (existingListing) {
        ...
        const newPrice = maxBuyPrice * (0.60 + Math.random() * 0.20);
        ...
        console.log(`    ✅ Updated ${seller.name}: ${existingListing.quantity} → ${50 + Math.floor(Math.random() * 150)} tons @ €${newPrice.toFixed(2)}/ton`);
      } else {
        // Create new listing
        const askingPrice = maxBuyPrice * (0.60 + Math.random() * 0.20);
        ...
        console.log(`    ✅ Created ${seller.name}: ${listing.quantity} tons @ €${listing.askingPrice}/ton`);
      }
    }
  }
