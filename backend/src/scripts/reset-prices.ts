...
async function main() {
  ...
  const priceRanges: { [key: string]: { buyMin: number; buyMax: number; sellMin: number; sellMax: number } } = {
  ...
  for (const listing of buyListings) {
    ...
    const newPrice = range.buyMin + Math.random() * (range.buyMax - range.buyMin);
    ...
    console.log(`  ✅ ${listing.buyer.name} - ${listing.product.name}: €${newPrice.toFixed(2)}/ton`);
  }
  ...
  for (const listing of saleListings) {
    ...
    const newPrice = range.sellMin + Math.random() * (range.sellMax - range.sellMin);
    ...
    console.log(`  ✅ ${listing.seller.name} - ${listing.product.name}: €${newPrice.toFixed(2)}/ton`);
  }
  ...
  for (const buyListing of updatedBuyListings) {
    ...
    const profitMargin = buyListing.maxPricePerUnit && matchingSales.length > 0
      ? ((buyListing.maxPricePerUnit.toNumber() - (matchingSales[0].askingPrice?.toNumber() || 0)) / buyListing.maxPricePerUnit.toNumber() * 100)
      : 0;
    ...
    if (matchingSales.length > 0) {
      console.log(`    Best seller price: €${matchingSales[0].askingPrice}/ton`);
  ...
  console.log(`💡 All prices are now within realistic ranges with good profit margins`);
}
