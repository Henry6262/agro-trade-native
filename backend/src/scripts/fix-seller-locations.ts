...
const bulgarianCities = [
  ...
  { name: "Dobrich", lat: 43.572, lng: 27.8273 },
  { name: "Sliven", lat: 42.6816, lng: 26.323 },
  { name: "Shumen", lat: 43.271, lng: 26.9361 },
  { name: "Pernik", lat: 42.6, lng: 23.0333 },
  { name: "Haskovo", lat: 41.934, lng: 25.5556 },
  ...
  { name: "Vidin", lat: 43.996, lng: 22.8679 },
  ...
  try {
    ...
    for (const listing of saleListings) {
      ...
      if (!listing.address) {
        ...
        const randomCity =
          bulgarianCities[Math.floor(Math.random() * bulgarianCities.length)];
        ...
        createdAddressCount++;
        console.log(
          `✅ Created address for listing ${listing.id}: ${randomCity.name} (${lat.toFixed(4)}, ${lng.toFixed(4)})`,
        );
        ...
        updatedCount++;
        console.log(
          `✅ Updated address for listing ${listing.id}: ${randomCity.name} (${lat.toFixed(4)}, ${lng.toFixed(4)})`,
        );
      } else {
        ...
        if (
          ...
          !listing.address.street.includes("Region")
        ) {
          await prisma.address.update({
            where: { id: listing.address.id },
            data: {
          ...
          updatedCount++;
          console.log(
            `✅ Updated city name for listing ${listing.id}: ${nearestCity.name}`,
          );
        }
      }
    }
    ...
    for (const seller of sellersWithoutAddresses) {
      ...
      createdAddressCount++;
      console.log(
        `✅ Created default address for seller ${seller.name}: ${randomCity.name}`,
      );
    }
