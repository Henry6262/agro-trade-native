...
async function main() {
  ...
  for (const listing of buyListings) {
    ...
    if (currentPrice > priceRange.max * 2 || currentPrice < priceRange.min / 2) {
      // Price is unrealistic, fix it
      const newPrice = priceRange.min + Math.random() * (priceRange.max - priceRange.min);
      ...
      console.log(`  ✅ Fixed ${listing.product.name}: €${currentPrice}/ton → €${newPrice.toFixed(2)}/ton`);
    }
  }
  ...
  for (const listing of saleListings) {
    ...
    if (currentPrice > priceRange.max * 2 || currentPrice < priceRange.min / 2) {
      ...
      const buyListingsForProduct = buyListings.filter(b => b.productId === listing.productId);
      ...
      if (buyListingsForProduct.length > 0) {
        ...
        const newPrice = maxBuyPrice * (0.70 + Math.random() * 0.10);
        ...
        console.log(`  ✅ Fixed ${listing.product.name} seller price: €${currentPrice}/ton → €${newPrice.toFixed(2)}/ton`);
      } else {
  ...
  for (const buyListing of updatedBuyListings) {
    ...
    console.log(`  ${buyListing.buyer.name} - ${buyListing.product.name}: ${matchingSales} matching sellers`);
  }
