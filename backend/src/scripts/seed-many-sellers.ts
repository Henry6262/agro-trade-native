...
async function main() {
  ...
  for (let i = 0; i < farmNames.length; i++) {
    ...
    try {
      ...
      for (let j = 0; j < numListings; j++) {
        ...
        const productBuyListings = buyListings.filter(
        ...
        if (productBuyListings.length > 0) {
          ...
          const maxBuyPrice = Math.max(
            ...productBuyListings.map(
              (b) => b.maxPricePerUnit?.toNumber() || 0,
            ),
          );
          askingPrice = maxBuyPrice * (0.6 + Math.random() * 0.2);
        } else {
          ...
          const priceRange =
            basePrices[product.name.toLowerCase().replace(" ", "_")] ||
            basePrices.other;
          askingPrice =
        ...
        if (!existingListing) {
          ...
          console.log(
            `    📦 ${product.name}: ${quantity} tons @ €${askingPrice.toFixed(2)}/ton`,
          );
        }
      }
    } catch (error) {
  ...
  for (const buyListing of buyListings.slice(0, 5)) {
    // Check first 5 buy listings
    ...
    console.log(
      `  ${buyListing.product.name}: ${matchingSales} matching sellers available`,
    );
  }
