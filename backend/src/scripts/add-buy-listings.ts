  ...
  try {
    ...
    for (const buyer of buyers) {
      ...
      if (existingListings > 0) {
        console.log(
          `⏭️  ${buyer.name} already has ${existingListings} active listings`,
        );
        continue;
      }
      ...
      for (let i = 0; i < numListings; i++) {
        ...
        listingCount++;
        console.log(
          `✅ Created listing for ${buyer.name}: ${quantity} ${product.defaultUnit} of ${product.name} @ max €${maxPrice}/unit`,
        );
      }
    }
    ...
    for (const buyer of buyers) {
      ...
      if (listings.length > 0) {
        ...
        listings.forEach((listing) => {
          console.log(
            `    - ${listing.quantity} ${listing.unit} of ${listing.product.name} @ max €${listing.maxPricePerUnit}/unit`,
          );
        });
      }
    }
    ...
    console.log(
      "You can now create trade operations with these active buy listings.",
    );
  } catch (error) {
