...
import { ApiTags, ApiBody, ApiOkResponse, ApiOperation, ApiBearerAuth } from '@nestjs/swagger';
...
export class AuthController {
  ...
  async googleMobileAuth(@Body() body: GoogleMobileAuthDto) {
    try {
      ...
      throw new BadRequestException(error?.message || 'Failed to authenticate with Google');
    }
  }
  ...
  async registerTransporter(@Body() dto: TransporterRegisterDto) {
    ...
    return this.serializeTransporterResult(authResult, {
  ...
  async getProfile(@CurrentUser() user: User) {
    const companyContext = await this.permissionsService.getUserCompanyContext(user.id);
