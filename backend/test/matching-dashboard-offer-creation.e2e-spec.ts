...
describe("Matching Dashboard - Offer Creation Workflow (e2e)", () => {
  ...
  async function setupTestData() {
    ...
    if (!product) {
      throw new Error(
        "SOFT_WHEAT product not found in database. Run seed first.",
      );
    }
  ...
  async function cleanupTestData() {
    ...
    await prisma.offerNegotiation.deleteMany({
      where: {
        tradeSeller: { tradeOperation: { buyListingId: testBuyListingId } },
      },
    });
    ...
    await prisma.address.deleteMany({
      where: {
        id: {
          in: [testBuyerAddressId, testSeller1AddressId, testSeller2AddressId],
        },
      },
    });
  ...
  describe("Complete Offer Creation Flow", () => {
    it("should successfully create trade operation with sellers (matching PricingModal flow)", async () => {
      ...
      // Verify seller 1
      const dbSeller1 = tradeOperation!.sellers.find(
        (s) => s.sellerId === testSeller1Id,
      );
      ...
      // Verify seller 2
      const dbSeller2 = tradeOperation!.sellers.find(
        (s) => s.sellerId === testSeller2Id,
      );
      ...
      const createdOperation = operations.find(
        (op: any) => op.id === tradeOperationId,
      );
    ...
    it("should handle negative profit scenario (low offer prices)", async () => {
      ...
      await prisma.offerNegotiation.deleteMany({
        where: {
          tradeSeller: {
            tradeOperation: { buyListingId: lowPriceBuyListing.id },
          },
        },
      });
      await prisma.tradeSeller.deleteMany({
