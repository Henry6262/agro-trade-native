...
class APITestRunner {
  ...
  async testTradeOperations() {
    ...
    // List operations
    const listResult = await this.runTest('GET', '/api/trade-operations?status=PENDING');
  ...
  async testProfitCalculations() {
    ...
    // Get benchmarks
    const benchmarksResult = await this.runTest('GET', '/api/profit/benchmarks');
  ...
  async testTransport() {
    ...
    const calculateResult = await this.runTest('POST', '/api/transport/calculate-cost', {
    ...
    // Optimize route
    const optimizeResult = await this.runTest('POST', '/api/transport/optimize-route', {
      origin: { lat: 42.6977, lng: 23.3219 },
      destinations: [
    ...
    const updateSettingsResult = await this.runTest('PUT', '/api/transport/settings', {
      baseCostPerKm: 2.5,
  ...
  async testNegotiations() {
    ...
    // Submit buyer offer
    const buyerOfferResult = await this.runTest('POST', '/api/negotiations/buyer-offer', {
    ...
    // Submit seller offer
    const sellerOfferResult = await this.runTest('POST', '/api/negotiations/seller-offer', {
    ...
    // Bulk negotiate
    const bulkNegotiateResult = await this.runTest('POST', '/api/negotiations/bulk-negotiate', {
      tradeOperationId: tradeOpId,
  ...
  async testScenarios() {
    ...
    // Create scenario
    const createResult = await this.runTest('POST', `/api/scenarios/${tradeOpId}/create`, {
  ...
  private printResult(result: TestResult) {
    ...
    if (result.status === 'PASS' && result.data) {
      ...
      if (result.data.profitMargin !== undefined) {
        console.log(chalk.gray(`   Profit Margin: ${result.data.profitMargin}%`));
      }
      if (result.data.estimatedProfit !== undefined) {
        console.log(chalk.gray(`   Estimated Profit: €${result.data.estimatedProfit}`));
      }
  ...
  private generateSummary() {
    ...
    const slowEndpoints = this.results.filter(r => r.responseTime && r.responseTime > 1000);
