...
describe('Transport Optimization Integration Tests', () => {
  ...
  describe('Transport Cost Estimation', () => {
    ...
    it('should apply vehicle-specific rates', async () => {
      ...
      const truckCost = results.find(r => r.vehicleType === 'TRUCK')?.totalCost || 0;
      const vanCost = results.find(r => r.vehicleType === 'VAN')?.totalCost || 0;
      const lorryCost = results.find(r => r.vehicleType === 'LORRY')?.totalCost || 0;
  ...
  describe('Route Optimization Algorithms', () => {
    ...
    it('should compare all algorithms and recommend best', async () => {
      ...
      const algorithmNames = response.body.comparison.algorithms.map((a: any) => a.name);
      ...
      expect(response.body.comparison.recommendation).toHaveProperty('algorithm');
  ...
  describe('Cost Breakdown Analysis', () => {
    it('should provide detailed cost breakdown', async () => {
      ...
      const totalPercentage = Object.values(percentages as Record<string, number>)
