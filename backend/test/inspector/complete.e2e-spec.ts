  ...
  describe('POST /api/inspector/jobs/:id/complete', () => {
    it('should submit verification results', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          ...
          expect(res.body.data).toHaveProperty('verificationStatus', 'VERIFIED');
          expect(res.body.data).toHaveProperty('verifiedSpecs');
          expect(res.body.data.verifiedSpecs).toEqual(verificationResult.verifiedSpecs);
        });
    });
    ...
    it('should require at least one test method', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          expect(res.body.success).toBe(false);
          expect(res.body).toHaveProperty('error', 'At least one test method is required');
        });
    });
    ...
    it('should require notes for FAILED status', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          expect(res.body.success).toBe(false);
          expect(res.body).toHaveProperty('error', 'Notes are required for failed verification');
        });
    });
    ...
    it('should only allow completion of IN_PROGRESS jobs', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          expect(res.body.success).toBe(false);
          expect(res.body).toHaveProperty('error', 'Job must be in progress to complete');
        });
    });
