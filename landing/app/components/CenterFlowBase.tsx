...
export interface CenterFlowProps {
  ...
  /** Override node background color (dark mode) */
  nodeBackground?: string;
  /** Override node border color (dark mode) */
  nodeBorder?: string;
  className?: string;
}
...
const CenterFlowBase: React.FC<CenterFlowProps> = ({
  ...
  useEffect(() => {
    ...
    const spawnPulseForPath = (pathIndex: number) => {
      setPulses((prev) => [
        ...prev,
        {
          id: `${pathIndex}-${Date.now()}-${Math.random()
            .toString(36)
            .substr(2, 9)}`,
          pathIndex,
          startTime: Date.now(),
  ...
  const nodeStyle = useMemo(
    () => ({
      ...
      background: nodeBackground ?? nodeBgColor,
      backdropFilter: "blur(8px)",
      border: `1px solid ${nodeBorder ?? (isLight ? "rgba(0,0,0,0.1)" : "rgba(255,255,255,0.1)")}`,
      boxShadow: isLight
        ? "0 2px 8px rgba(0,0,0,0.1)"
        : "0 2px 8px rgba(0,0,0,0.3)",
    }),
    [borderRadius, isLight, nodeBgColor, nodeBackground, nodeBorder]
  );
...
CenterFlowBase.displayName = "CenterFlowBase";
export default CenterFlowBase;
