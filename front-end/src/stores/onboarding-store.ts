...
export interface FleetInfo {
  vehicleCount: number
  vehicleTypes: TruckInfo[]
  capacity: { total: number; unit: string }
  baseLocation: {
    id: string
    address: string
    city: string
...
export interface OnboardingState {
  ...
  setFleetInfo: (fleetInfo: Partial<FleetInfo>) => void
...
export const useOnboardingStore = create<OnboardingState>()(
  devtools(
    (set, get) => ({
      ...
      setUserType: (type) => {
        console.log("[OnboardingStore] Setting user type:", type)
        set({ userType: type })
      },

      setSelectedProducts: (products) => {
        console.log("[OnboardingStore] Setting selected products:", products)
