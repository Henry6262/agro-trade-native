...
import { View, Text, TouchableOpacity, Image, ActivityIndicator, Alert, Dimensions, ScrollView } from 'react-native';
...
export const ProductSelectionUnified: React.FC = () => {
  ...
  const toggleProduct = (productId: string) => {
    ...
    if (role === 'seller' || role === 'buyer') {
      ...
      // Update metadata with product specifications
      setSelectedProductsMetadata(
        productSelectionService.buildMetadata(products, newSelection)
      );
    }
  };
            ...
            {Array.from({ length: Math.ceil(products.length / 2) }, (_, rowIndex) => (
              <View key={rowIndex} className="flex-row mb-3">
                {products.slice(rowIndex * 2, rowIndex * 2 + 2).map((product, colIndex) => {
                  const isSelected = selectedProductIds.includes(product.id) || selectedProductIds.includes(product.category);
                    ...
                    >
                      ...
                      {product.image && (
                        <View className="relative w-full h-32">
                          <Image
                            source={{ 
                              uri: productSelectionService.resolveImageUri(product.image || null) ?? undefined,
                            }}
