...
import { GradientBackground, GlassCard, GlassBadge, GlassButton, COLORS } from '../../design-system';
...
export default function UnifiedDashboardScreen() {
  ...
  const navigateToProduct = (productId: string) => navigation.navigate('TradeDetail', { tradeId: productId } as any);
  ...
  const renderSellerView = () => (
    <>
      ...
      <View style={styles.statsRow}>
        <StatBox value={offers.filter((o) => (o.status || '').toLowerCase() === 'pending').length} label="New Offers" icon={<Bell size={16} color={COLORS.accentGold} />} />
        <StatBox value={trades.length} label="Active Trades" icon={<Package size={16} color="#4ADE80" />} />
        <StatBox value={trades.filter((t) => (t.status || '').toLowerCase() === 'complete').length} label="Completed" icon={<TrendingUp size={16} color="#4ADE80" />} />
      </View>

      {/* Offers Section */}
      <SectionHeader title="Incoming Offers" count={offers.length} />
      {offers.filter((o) => (o.status || '').toLowerCase() === 'pending').slice(0, 3).map((offer) => (
        <GlassCard key={offer.id} tier="medium" style={styles.offerCard} animate={false}>
          ...
          <Text style={styles.offerMeta}>{offer.buyer} • {offer.quantity} tons @ ${offer.offeredPricePerTon}/ton</Text>
          <Text style={styles.offerValue}>Total: <Text style={{ color: COLORS.accentGold, fontWeight: '800' }}>${offer.totalValue.toLocaleString()}</Text></Text>
          <View style={styles.offerActions}>
            <GlassButton label="Reject" onPress={() => handleReject(offer.negotiationId)} variant="ghost" size="sm" style={styles.actionBtn} disabled={actionLoading === offer.negotiationId} />
            <GlassButton label="Accept" onPress={() => handleAccept(offer.negotiationId)} variant="primary" size="sm" style={styles.actionBtn} disabled={actionLoading === offer.negotiationId} />
          </View>
        </GlassCard>
      ))}
      {offers.filter((o) => (o.status || '').toLowerCase() === 'pending').length === 0 && (
        <EmptyState title="No pending offers" subtitle="Buyers will send offers here." icon={<Bell size={28} color="rgba(255,255,255,0.2)" />} />
      )}

      {/* Trades Section */}
      <SectionHeader title="Active Trades" count={trades.length} />
      {trades.slice(0, 5).map((trade) => (
        <TouchableOpacity key={trade.id} onPress={() => navigateToTrade(trade.id)} activeOpacity={0.8}>
          <GlassCard tier="subtle" style={styles.tradeCard} animate={false}>
            <View style={styles.tradeRow}>
              ...
              <View style={styles.tradeInfo}>
                ...
                <Text style={styles.tradeMeta}>{trade.buyerName || trade.buyer?.name} • {trade.agreedQuantity || trade.quantity} {trade.unit}</Text>
              </View>
  ...
  const renderBuyerView = () => (
    <>
      <View style={styles.statsRow}>
        <StatBox value={orders.length} label="Orders" icon={<ShoppingCart size={16} color={COLORS.accentGold} />} />
        <StatBox value={orders.filter((o) => (o.status || '').toLowerCase() === 'pending').length} label="Pending" icon={<Bell size={16} color="#FCD34D" />} />
        <StatBox value={orders.filter((o) => (o.status || '').toLowerCase() === 'delivered').length} label="Delivered" icon={<TrendingUp size={16} color="#4ADE80" />} />
      </View>

      <SectionHeader title="Browse Marketplace" count={products.length} />
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.productScroll}>
        {products.slice(0, 8).map((product) => (
          <TouchableOpacity key={product.id} onPress={() => navigateToProduct(product.id)} activeOpacity={0.8}>
            <GlassCard tier="medium" style={styles.productCard} animate={false}>
              <View style={styles.productImage}>
                <Wheat size={24} color="rgba(255,255,255,0.2)" />
              </View>
              <Text style={styles.productCardName} numberOfLines={1}>{product.name}</Text>
              <Text style={styles.productCardPrice}>${product.price}/{product.unit}</Text>
              <Text style={styles.productCardSeller} numberOfLines={1}>{product.seller?.name}</Text>
            </GlassCard>
          </TouchableOpacity>
        ))}
      </ScrollView>
        ...
        <GlassCard key={order.id} tier="subtle" style={styles.tradeCard} animate={false}>
          <View style={styles.tradeRow}>
            ...
            <View style={styles.tradeInfo}>
              ...
              <Text style={styles.tradeMeta}>{order.items?.length || 0} items • ${order.totalAmount}</Text>
            </View>
      ...
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <View style={styles.roleBadge}>
            ...
            <Text style={styles.roleText}>{isSeller ? 'SELLER' : isBuyer ? 'BUYER' : 'TRANSPORTER'}</Text>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  roleBadge: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: 'rgba(74,222,128,0.12)', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 20, borderWidth: 1, borderColor: 'rgba(74,222,128,0.25)' },
  roleText: { color: COLORS.accentGreen, fontSize: 10, fontWeight: '700', letterSpacing: 0.5 },
  scroll: { flex: 1 },
  scrollContent: { padding: 16, paddingBottom: 40 },
  statsRow: { flexDirection: 'row', gap: 10, marginBottom: 20 },
  statBox: { flex: 1, alignItems: 'center', padding: 12, gap: 6 },
  statValue: { color: '#fff', fontSize: 20, fontWeight: '800' },
  statLabel: { color: COLORS.textMuted, fontSize: 10, fontWeight: '600', textTransform: 'uppercase' },
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 8, marginBottom: 12 },
  sectionTitle: { color: 'rgba(255,255,255,0.6)', fontSize: 13, fontWeight: '700', letterSpacing: 0.8, textTransform: 'uppercase' },
  sectionCount: { color: COLORS.accentGold, fontSize: 13, fontWeight: '700' },
  offerCard: { padding: 14, marginBottom: 10 },
  offerHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  offerProduct: { color: '#fff', fontSize: 15, fontWeight: '700', flex: 1 },
  ...
  actionBtn: { flex: 1 },
  tradeCard: { padding: 12, marginBottom: 8 },
  ...
  productCard: { width: 140, padding: 12, marginRight: 10, alignItems: 'center' },
  productImage: { width: 60, height: 60, borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.06)', alignItems: 'center', justifyContent: 'center', marginBottom: 10 },
  ...
  productCardSeller: { color: 'rgba(255,255,255,0.4)', fontSize: 11, marginTop: 2, textAlign: 'center' },
});
