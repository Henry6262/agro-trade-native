...
export function AdminProductPricesScreen() {
  ...
  const categories = ['WHEAT', 'CORN', 'SUNFLOWER', 'BARLEY', 'OATS', 'RAPESEED', 'PEAS', 'SOYBEAN_MEAL', 'WHEAT_BRAN', 'ALFALFA']
  ...
  const createProductPrice = async () => {
    if (!newPrice.productId || !newPrice.pricingZoneId || !newPrice.minPrice || !newPrice.maxPrice) {
  ...
  const filteredPrices = productPrices.filter(price => {
    const matchesSearch = price.product.displayName.toLowerCase().includes(searchTerm.toLowerCase()) ||
