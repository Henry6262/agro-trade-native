...
interface TradeCreationDrawerProps {
  ...
  createTradeOperation: (buyListingId: string, targetMargin: number) => Promise<TradeOperation | null>;
...
export const TradeCreationDrawer: React.FC<TradeCreationDrawerProps> = ({
  ...
  const handleCreateTrade = async () => {
    ...
    if (trade) {
      ...
      Alert.alert('Success', 'Trade operation created! You can now select sellers and send offers.');
      setCurrentStep(2);
  ...
  const handleSelectSellers = async () => {
    ...
    const sellersToAdd = selectedSellers
      .map(sellerId => {
        ...
        console.log(`Seller ${seller.sellerId}: Available: ${sellerAvailability}, Taking: ${requestedQty}, Remaining needed: ${remainingQuantity}`);
  ...
  const handleSendOffers = async () => {
    ...
    const sellerOffers = selectedSellers.map(sellerId => {
      const seller = matchingSellers.find(s => s.sellerId === sellerId);
      return {
        sellerId,
  ...
  const canProceed = () => {
    switch (currentStep) {
      case 1: return !!buyListing;
      case 2: return selectedSellers.length > 0;
      case 3: return !!transportEstimate;
      case 4: return !!profitCalculation;
      case 5: return true;
      ...
      {!tradeOperation && (
        <View className="bg-blue-50 rounded-lg p-3 mb-4 border border-blue-200">
          ...
          <Text className="text-blue-700 text-sm">
            Clicking "Create Trade & Continue" will create a trade operation with your specified margin. 
            You'll then be able to select sellers and send offers immediately.
          </Text>
        </View>
      )}
            ...
            {buyListing.deliveryAddress && (
              <View className="flex-row items-center mt-1">
                ...
                <Text className="text-gray-500 text-xs ml-1">
                  {buyListing.deliveryAddress.city || buyListing.deliveryAddress.address || 'Delivery location set'}
                </Text>
              </View>
            )}
          </View>
        ...
        <View className="px-4 pt-4">
          <View className="bg-blue-50 p-3 rounded-lg border border-blue-200 mb-4">
            <View className="flex-row justify-between items-center mb-2">
              <View className={`px-2 py-1 rounded ${progressPercentage >= 100 ? 'bg-green-100' : 'bg-orange-100'}`}>
                <Text className={`text-sm font-bold ${progressPercentage >= 100 ? 'text-green-800' : 'text-orange-800'}`}>
            ...
            {buyListing?.specifications && buyListing.specifications.length > 0 && (
              <View className="mt-2 pt-2 border-t border-blue-200">
                <Text className="text-gray-600 text-sm mb-1 font-medium">Required Specifications:</Text>
            ...
            <>
                ...
                return (
                  ...
                  >
                        ...
                        <View className="flex-row items-center mt-1">
                          ...
                          <Text className="text-gray-500 text-xs ml-1">
                            {seller.location?.displayName || 
                             (seller.location?.city ? `${seller.location.city} • ${seller.distance}km` : 
                              'Location N/A')}
                          </Text>
                        </View>
                      </View>
                    ...
                    <View className="mb-2">
                      ...
                      {isSelected && (
                        <Text className={`text-sm font-semibold mt-1 ${isPartialUse ? 'text-orange-600' : 'text-green-600'}`}>
                          {isPartialUse 
              ...
              {matchingSellers.length === 0 && (
                <View className="py-8 items-center">
                  ...
                  <Text className="text-gray-400 text-sm mt-2">Try refreshing or adjusting criteria</Text>
                </View>
              )}
            </>
          )}
        ...
        <>
          <View className="bg-white rounded-lg p-4 border border-gray-200 mb-4">
            ...
            <View className="flex-row justify-between mb-2">
              ...
              <Text className="font-semibold">
                {profitCalculation.revenue.quantity} units
              </Text>
            </View>
    ...
    return (
      <ScrollView className="flex-1 p-4">
        ...
        <View className="bg-white rounded-lg p-4 border border-gray-200 mb-4">
          ...
          <View className="mb-2">
            <Text className="text-gray-600 text-sm">To: {buyListing.buyer?.name || 'Unknown Buyer'}</Text>
            ...
            return (
              <View key={sellerId} className="mb-4 pb-4 border-b border-gray-100 last:border-0">
                ...
                <TextInput
                  ...
                  onChangeText={(value) => setOfferPrices(prev => ({
                    ...prev,
        ...
        {profitCalculation && (
          <View className="bg-yellow-50 rounded-lg p-3 mb-4">
            <Text className="text-yellow-800 text-sm">
              Expected profit with current prices: ${profitCalculation.profit.netProfit.toFixed(2)} ({profitCalculation.profit.profitMargin.toFixed(1)}%)
            </Text>
          </View>
        )}
      </ScrollView>
            ...
            <View className="flex-1">
              ...
              {buyListing && (
                <Text className="text-sm text-gray-600">
                  {buyListing.product?.name || 'Unknown Product'} - {buyListing.quantity} {buyListing.unit || 'TON'}
                </Text>
              )}
            </View>
              ...
              {(isCreatingTrade || isSendingOffers) ? (
                ...
                <>
                  <Text className="text-white font-semibold mr-2">
                    {currentStep === 1 ? 'Create Trade & Continue' : currentStep === 5 ? 'Send Offers' : 'Next'}
                  </Text>
