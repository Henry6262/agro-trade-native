...
describe('Map Drawer', () => {
  ...
  it('should animate drawer sliding up from bottom', async () => {
    const { rerender } = render(<MapDrawer isOpen={false} offer={mockOffer} onClose={jest.fn()} />);
  ...
  it('should prevent scrolling the background when drawer is open', () => {
    const { container } = render(<MapDrawer isOpen={true} offer={mockOffer} onClose={jest.fn()} />);
